...
import { u, A } from './index-BJoWiIN4.mjs';
...
const F = () => Promise.resolve().then(() => y), b = A("app_routes_deferred_tsx--personServerFn_createServerFn_handler", "/_server", (e) => l.__executeServer(e)), l = u({ method: "GET" }).validator((e) => e).handler(b, ({ data: e }) => ({ name: e, randomNumber: Math.floor(Math.random() * 100) })), w = A("app_routes_deferred_tsx--slowServerFn_createServerFn_handler", "/_server", (e) => m.__executeServer(e)), m = u({ method: "GET" }).validator((e) => e).handler(w, async ({ data: e }) => (await new Promise((n) => setTimeout(n, 1e3)), { name: e, randomNumber: Math.floor(Math.random() * 100) })), p = createFileRoute("/deferred")({ loader: async () => ({ deferredStuff: new Promise((e) => setTimeout(() => e("Hello deferred!"), 2e3)), deferredPerson: m({ data: "Tanner Linsley" }), person: await l({ data: "John Doe" }) }), component: lazyRouteComponent(F, "component", () => p.ssr) }), x = function() {
...
//# sourceMappingURL=deferred-BRTn-SDt.mjs.map
